...
public final class ExpUtil {
    ...
    // not my code, credits: DOGC_Kyle on spigot

    // Calculate amount of EXP needed to level up
    public int getExpToLevelUp(final int level) {
        if (level <= 15) {
            return 2 * level + 7;
        } else if (level <= 30) {
            return 5 * level - 38;
    ...
    // Give or take EXP
    public int addPlayerExp(final Player player, int exp) {
    ...
    public int removePlayerExp(final Player player, int exp) {
        // Get player's current exp
        int currentExp = getPlayerExp(player);
